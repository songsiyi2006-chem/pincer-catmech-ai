...
def kill_process_tree(parent, proc):
    """Always attempt parent kill even when a previously listed child vanishes."""
    errors = []
    try:
...
def main():
    ...
    ap.add_argument("--rss-limit-mib", type=int, choices=[768, 896, 1024], default=1024)
    ap.add_argument("--soscf", action="store_true", help="Use one documented second-order SCF rescue without loosening convergence tolerances")
    ...
    cfg = {
        ...
        "timeout_seconds": 300, "process_tree_rss_limit_bytes": args.rss_limit_mib * 1024**2,
        "sampling_interval_seconds": 0.1,
        "geometry_optimization": False, "frequency_calculation": False,
        "soscf_rescue": args.soscf,
    }
    ...
    if available < 2 * cfg["process_tree_rss_limit_bytes"]:
        record.update(status="resource_preflight_not_launched", reason="available RAM below twice sampled RSS limit")
        (root / "run_record.json").write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
        raise SystemExit(f"Insufficient free RAM for child limit: {available} bytes available")
    ...
    try:
        with (root / "stdout.log").open("w", encoding="utf-8") as stdout, (root / "stderr.log").open("w", encoding="utf-8") as stderr:
            ...
            while proc.poll() is None:
                ...
                if termination:
                    record["termination_errors"] = kill_process_tree(parent, proc)
                    break
                time.sleep(cfg["sampling_interval_seconds"])
    except Exception as exc:
        termination = "launch_failed" if proc is None else "monitor_error"
        record["exception"] = repr(exc)
    finally:
        if proc is not None:
            try:
                if proc.poll() is None:
                    record.setdefault("termination_errors", []).extend(kill_process_tree(parent, proc))
                exit_code = proc.wait(timeout=10)
            except Exception as exc:
                termination = termination or "monitor_error"
                record["final_wait_exception"] = repr(exc)
                try:
                    proc.kill()
                    exit_code = proc.wait(timeout=2)
                except Exception as final_exc:
                    record["final_kill_exception"] = repr(final_exc)
        record.update(
        ...
        record["files"] = {
            ...
            if p.is_file() and "scratch" not in p.parts and not p.name.startswith("psi.")
        }
        (root / "run_record.json").write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
