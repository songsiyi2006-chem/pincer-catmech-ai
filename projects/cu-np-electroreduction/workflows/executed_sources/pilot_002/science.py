...
def site_steady_state(rates):
    ...
    """
    if len(rates) != 6 or min(rates) <= 0 or not all(map(math.isfinite,rates)):
        raise ValueError("Six finite positive rates required")
    a,b,c,d,e,f = rates
    weights=[b*e+b*d+c*e, a*d+a*e+f*d, f*c+a*c+b*f]
    theta=[x/sum(weights) for x in weights]
    residual=[(-a-f)*theta[0]+b*theta[1]+e*theta[2],a*theta[0]-(b+c)*theta[1]+d*theta[2],f*theta[0]+c*theta[1]-(d+e)*theta[2]]
    if min(theta) < -1e-10 or max(map(abs,residual)) > 1e-8 * max(rates):
