...
def source_fingerprints():
    """Code present at this invocation; never silently relabel an older run."""
    return {str(path.relative_to(REPO)): digest(path.read_bytes()) for path in
            (Path(__file__), REPO / "src/pincer_catmech/models/pincer_egnn.py",
             REPO / "tests/test_pincer_egnn.py")}
...
def run_training(output: Path, *, seed=20260913, epochs=30, batch_size=4, hidden_dim=24,
    ...
    import psutil
    training_sources = source_fingerprints()
    ...
    torch.save({"model_state_dict": model.state_dict(), "configuration": configuration,
                "normalization_mean": normalizer.mean, "normalization_scale": normalizer.scale,
                 ...
                 "training_source_files_sha256": training_sources,
...
def verify_benchmark_evidence(output: Path, report: dict) -> tuple[dict, dict, dict]:
    """Bind a summary refresh to its trained data and checkpoint before writing.

    Hash agreement establishes artifact identity, not scientific truth. Current
    source edits are allowed: training and verification provenance are separate.
    """
    manifest_bytes = (output / "dataset_manifest.json").read_bytes()
    if digest(manifest_bytes) != report["dataset_manifest_sha256"]:
        raise ValueError("Training dataset manifest SHA mismatch")
    manifest = json.loads(manifest_bytes)
    for filename, expected in manifest["dataset_files_sha256"].items():
        if digest((output / filename).read_bytes()) != expected:
            raise ValueError(f"Training dataset file SHA mismatch: {filename}")
    run = output / report["run_directory"].replace("\\", "/")
    split_bytes = (run / "split.json").read_bytes()
    if digest(split_bytes) != report["split_sha256"]:
        raise ValueError("Training split SHA mismatch")
    ...
    if checkpoint["dataset_manifest_sha256"] != report["dataset_manifest_sha256"]:
        raise ValueError("Checkpoint is bound to a different training dataset manifest")
    if checkpoint["configuration"] != report["configuration"]:
        raise ValueError("Checkpoint configuration disagrees with training report")
    for key in ("mean", "scale", "supported", "counts"):
        actual = checkpoint["normalization_" + key]
        expected = torch.as_tensor(report["normalization"][key], dtype=actual.dtype)
        if not torch.equal(actual, expected):
            raise ValueError(f"Checkpoint normalization {key} disagrees with training report")
    heads = checkpoint["configuration"]["heads"]
    for key, field in (("trained_heads", "trained"), ("pair_only_heads", "pair_only")):
        expected = torch.tensor([bool(report["heads"][head].get(field, False)) for head in heads])
        if not torch.equal(checkpoint["model_state_dict"][key], expected):
            raise ValueError(f"Checkpoint {key} disagree with training readiness")
    archived_sources = checkpoint.get("training_source_files_sha256")
    if archived_sources is not None and archived_sources != report.get("training_source_files_sha256"):
        raise ValueError("Checkpoint training-source provenance disagrees with training report")
    return manifest, checkpoint, json.loads(split_bytes)


def training_source_provenance(output: Path, report: dict, checkpoint: dict) -> tuple[dict | None, str, dict]:
    """Retain initial-run hashes when verifying a legacy checkpoint with new code."""
    previous_path = output / "summary.json"
    previous = json.loads(previous_path.read_bytes()) if previous_path.exists() else {}
    binding = ("model_sha256", "dataset_manifest_sha256", "split_sha256")
    if not all(previous.get(key) == report[key] for key in binding):
        previous = {}
    if checkpoint.get("training_source_files_sha256") is not None:
        return checkpoint["training_source_files_sha256"], "captured before training and embedded in checkpoint", previous
    retained = previous.get("training_source_files_sha256") or previous.get("source_files_sha256")
    if retained:
        return retained, "retained from the existing summary with matching checkpoint, manifest and split hashes", previous
    return None, "unavailable for this legacy run; current verifier code is not training provenance", previous
...
def summarize_benchmark(output: Path) -> dict:
    ...
    if report["status"] != "auxiliary_benchmark_trained":
        ...
        return summary
    manifest, checkpoint, split = verify_benchmark_evidence(output, report)
    training_sources, source_origin, previous = training_source_provenance(output, report, checkpoint)
    ...
    summary["finished_utc"] = previous.get("finished_utc", utc())
    summary["verification_utc"] = utc()
    summary["computation_finished"] = True
    summary["split"] = {name: {"pairs": len(values["indices"]), "family_ids": values["family_ids"]}
                        for name, values in split.items()}
    summary["source_files_sha256"] = training_sources  # Legacy field retains its original run meaning.
    summary["training_source_files_sha256"] = training_sources
    summary["training_source_provenance"] = source_origin
    summary["verification_source_files_sha256"] = source_fingerprints()
    summary["evidence_binding"] = {"verified": True, "checkpoint_manifest_dataset_split": True,
                                   "normalizer_configuration_and_head_readiness": True,
                                   "scope": "Artifact consistency; not chemical or predictive validation"}
    atomic_json(output / "summary.json", summary)
    return summary
...
def main():
    ...
    mode.add_argument("--verify-only", action="store_true",
                      help="Verify retained benchmark artifacts and refresh the summary without preparing data or training")
