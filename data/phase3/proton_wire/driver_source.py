...
def continue_first_attempt(args):
    ...
    for filename in ("summary.json", "native_manifest.json", "native_evidence.zip"):
        shutil.copy2(output / filename, output / ("before_continuation_" + filename))
    ...
    try:
        ...
        summary.update(finished_utc=utc_now(), elapsed_seconds=time.time()-(summary["deadline_epoch"]-args.budget_seconds))
